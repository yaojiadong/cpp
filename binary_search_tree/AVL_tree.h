...
protected:
  int height(const TPos &p) const {
    return (p.is_external() ? 0 : p->height());
  } // p is the pointer to the AVL_entry, p->_height
  ...
  bool isBalanced(const TPos &p) const {
  ...
  void rebalance(const TPos &p) {
    ...
    while (
        z !=
        ST::root()) { // rebalance up to root, without ST::, root is ambiguous.
                      // In the Linked_binary_tree, there is also root()
      z = z.parent();
      set_height(z);
      if (!isBalanced(z)) {
        TPos x = tallest_grandchild(z);
        z = ST::restructure(
            x); // restructure is defined in the base linked binary tree.
        set_height(z.left());
        set_height(z.right());
        set_height(z);
      }
