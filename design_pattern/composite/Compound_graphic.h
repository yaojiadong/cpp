...
public:
  ...
  void remove(std::shared_ptr<IGraphic> graphic) {
    graphics.erase(std::find(graphics.begin(), graphics.end(), graphic));
  }

private:
  std::vector<std::shared_ptr<IGraphic>> graphics;
  // graphics stores IGraphics: dot, circle, and compound_graphic
  // itself
};
